...
// RUN: %clang_link_purecap_bfd -external-capsizefix -Wl,-no-process-cap-relocs %t.o -static -o %t-static.exe
...
// RUN: %cheri_purecap_clang %legacy_caprelocs_flag %S/../Inputs/dummy_shlib.c -c -o %T/bfd_dummy_shlib.o
// RUN: %clang_link_purecap_bfd -external-capsizefix %T/bfd_dummy_shlib.o -shared -o %T/libbfd_dummy_shlib.so
// RUN: %clang_link_purecap_bfd -external-capsizefix -L %T -lbfd_dummy_shlib %t.o -o %t-dynamic.exe
...
// RUN: %clang_link_purecap_bfd -external-capsizefix -Wl,-no-process-cap-relocs %t.o -shared -o %t.so
