...
static uint64_t getTargetSize(const CheriCapRelocLocation &Location,
  ...
  if (TargetSize == 0 && !TargetSym->IsPreemptible) {
    ...
    bool WasWeakSym = !TargetSym->isLocal() && TargetSym->symbol()->WasUndefWeak;
    if (WasWeakSym)  {
      if (TargetSym->getVA(0) != 0) {
        ...
        error("Zero size weak symbol did not resolve to NULL but 0x" +
              ...
              verboseToString<ELFT>(TargetSym, 0));
      }
      return 0;
    }
    ...
    if (DefinedRegular *DefinedSym =
      dyn_cast<DefinedRegular>(TargetSym)) {
...
template <class ELFT> void CheriCapRelocsSection<ELFT>::writeTo(uint8_t *Buf) {
  ...
  for (const auto &I : RelocsMap) {
    ...
    // if (TargetOffset <= TargetSize) warn(...);
